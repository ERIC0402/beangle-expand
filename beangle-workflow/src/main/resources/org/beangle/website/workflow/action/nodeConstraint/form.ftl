[#ftl]
[@b.head/]
[@b.toolbar title="步骤节点限制"]bar.addBack();[/@]
[@b.tabs]
	[@b.tab label="步骤节点限制"]
		[@b.form action="!save" title="基本信息" theme="list"]
			[@b.field label="所属节点" ]
				<select name="nodeConstraint.stepNode.id" id="nodeConstraint.stepNode.id" style="width:160px;">
	   	  			[#if nodeConstraint.id??]
						<option selected value="${nodeConstraint.stepNode.id}">${nodeConstraint.stepNode.name}</option>
					[#else]
