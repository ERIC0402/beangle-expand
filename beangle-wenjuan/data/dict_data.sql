...
--考试类别
insert into SYS_DICT_TYPES (name, code, enabled, id)
     values ('在线考试类别', 'ZXKS_CATEGORY', 1, seq_sys_dict_types.nextval);
insert into SYS_DICT_DATAS (name, code, enabled, id, Dict_Type_Id)
     values ('试卷', 'ZXKS_CATEGORY_SHI_JUAN', 1, seq_sys_dict_datas.nextval, 
     (select max(id) from sys_dict_types where code = 'ZXKS_CATEGORY'));
insert into SYS_DICT_DATAS (name, code, enabled, id, Dict_Type_Id)
     values ('评教', 'ZXKS_CATEGORY__PING_JIAO', 1, seq_sys_dict_datas.nextval, 
     (select max(id) from sys_dict_types where code = 'ZXKS_CATEGORY'));
insert into SYS_DICT_DATAS (name, code, enabled, id, Dict_Type_Id)
     values ('调查', 'ZXKS_CATEGORY_DIAO_CHA', 1, seq_sys_dict_datas.nextval, 
     (select max(id) from sys_dict_types where code = 'ZXKS_CATEGORY'));
--考试类型
insert into SYS_DICT_TYPES (name, code, enabled, id)
     values ('在线考试类型', 'ZXKS_TYPE', 1, seq_sys_dict_types.nextval);
insert into SYS_DICT_DATAS (name, code, enabled, id, Dict_Type_Id)
     values ('正式考试', 'ZXKS_TYPE_ZSKS', 1, seq_sys_dict_datas.nextval, 
     (select max(id) from sys_dict_types where code = 'ZXKS_TYPE'));
insert into SYS_DICT_DATAS (name, code, enabled, id, Dict_Type_Id)
     values ('模拟考试', 'ZXKS_TYPE_MNKS', 1, seq_sys_dict_datas.nextval, 
     (select max(id) from sys_dict_types where code = 'ZXKS_TYPE'));