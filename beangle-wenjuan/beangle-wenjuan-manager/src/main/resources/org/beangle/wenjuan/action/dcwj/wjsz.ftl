...
<div style="padding: 10px; margin: 5px;" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
	...
	[@lc.table]
		[@b.formfoot]
			<input type="hidden" name="wenJuanDc.id" value="${wenJuanDc.id}" />
			[@b.redirectParams/]
...
<tr>
		<td>
			...
			<input type="hidden" name="wjsz${index}.sswj.id" value="${wenJuanDc.id}" />
		...
		<td><select name="wjsz${index}.tmlx.id">
				...
				[#list tmlxs as v]
					...
					[/#if]
				[/#list]</select></td>
		<td align="center"><input name="wjsz${index}.tmsl" style="width: 50px;" value="${wjsz.tmsl!}"/>[#if (wjsz.id)??]/<label title="题库中可使用的题目数">${wjsz.tiMus?size}</label>[/#if]</td>
...
<script language="JavaScript">
	...
	function validateForm(){
		...
		mtfs.each(function (){
			if(!/^(100|[1-9]?\d(\.\d{2})?)$/.test(this.value)){
				this.focus();
