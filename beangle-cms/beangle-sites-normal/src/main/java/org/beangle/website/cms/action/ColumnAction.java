...
public class ColumnAction extends SiteCommonAction {
	...
	public String listandform() {
		...
		put("siteId",siteId);
		if ((list.size() > 0 && !"yes".equals(self)) || column.getColumns()== null) {
			return forward("list");
		} else {
			//网页面上传送信息
			putEditDatas(siteId);
	...
	private void putEditDatas(Long siteId){
		...
		//入口
		put("entrys", entryService.find(siteId));
		...
		// 显示模板
		put("templates",entityDao.searchHQLQuery("from org.beangle.website.cms.model.Template"));
	}
	...
	public String save() {
		...
		if ("0".equals(get("column.site.id"))) {
			Site site = (Site) entityDao.searchHQLQuery(
					"from Site where name ='" + column.getName() + "'").get(0);
			column.setSite(site);
			column.setNode(true);
		} else {
			Site site = (Site) entityDao.get(Site.class,
					getLong("column.site.id"));
			column.setSite(site);
		...
		if(parentid != null){
			...
			if(column.getId() != null){
				if(hasChildColumn(column.getId())){
					column.setNode(true);
				}else{
					column.setNode(false);
				}
		...
		Long oldParentId = getLong("oldParentId");
		if(oldParentId != null && !oldParentId.equals(id)){
			Column oldParent = (Column) entityDao.get(Column.class, oldParentId);
			if(!hasChildColumn(oldParent.getId())){
				oldParent.setNode(false);
				entityDao.saveOrUpdate(oldParent);
			}
			Column parent = (Column) entityDao.get(Column.class, id);
			if(hasChildColumn(parent.getId())){
				parent.setNode(true);
		...
		//this.id = id;
		OperationLog ol = new OperationLog(getCurrentUser(), "栏目管理-新建栏目",
				new Timestamp(System.currentTimeMillis()), getRemoteAddr(),
				"新建了一个栏目" + column.getName());
		entityDao.saveOrUpdate(ol);
		return redirect("listandform", "info.save.success","column.id="+id);
	}
	...
	public void checkOrder() {
		...
		List list = entityDao.searchHQLQuery("from Column where orders='"
				+ order + "'");
	...
	public String remove() {
		...
		OperationLog ol = new OperationLog(getCurrentUser(), "栏目管理-删除栏目",
				new Timestamp(System.currentTimeMillis()), getRemoteAddr(),
				"删除了一个栏目" + cols.getName());
		entityDao.saveOrUpdate(ol);
		return redirect("listandform", "info.delete.success","column.id="+parent.getId());
	}
	
	protected boolean hasChildColumn(Long parentId){
		OqlBuilder<Column> oql = OqlBuilder.from(Column.class,"c");
		oql.where("c.columns.id=:parentId",parentId);
		List<Column> list = entityDao.search(oql);
		if(list.isEmpty()){
			return false;
		}else{
			...
			for(int i=0;i<cols.size();i++){
				...
				if(column.getId().equals(col.getId())){
					if(i != (cols.size() -1)){
						...
						List<Column> toUps = entityDao.searchHQLQuery("from org.beangle.website.cms.model.Column c where c.orders like '"+column.getOrders()+"%' and c.orders !='"+column.getOrders()+"'");
						...
						entityDao.saveOrUpdate(column);
						return redirect("listandform","move.up.success","column.id="+column.getColumns().getId());
					}
				}
			}
		}
		return redirect("listandform","can.not.move","column.id="+column.getColumns().getId());
	}
	
	/**
	 * 为栏目设置排序
	 * @param Column 要设置排序的栏目集合
	 * @param order 要设置的排序
	 */
			...
			for(int i=0;i<cols.size();i++){
				...
				if(column.getId().equals(col.getId())){
					if(i != 0){
						...
						List<Column> downs = entityDao.searchHQLQuery("from org.beangle.website.cms.model.Column c where c.orders like '"+down.getOrders()+"%' and c.orders !='"+down.getOrders()+"'");
						List<Column> toDowns = entityDao.searchHQLQuery("from org.beangle.website.cms.model.Column c where c.orders like '"+column.getOrders()+"%' and c.orders !='"+column.getOrders()+"'");
						...
						entityDao.saveOrUpdate(column);
						return redirect("listandform","move.down.success","column.id="+column.getColumns().getId());
					}
				}
			}
		}
