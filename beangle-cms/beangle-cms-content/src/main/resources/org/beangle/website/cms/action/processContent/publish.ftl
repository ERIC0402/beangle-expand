	...
	[@b.tab label="发布"]
		[@b.form action="!save" theme="list"]
			[#include "../audit.ftl"/]
	      	...
	      	<div class="button_box">
				<input type="button" value="发布" onClick="save(this.form);"/>
				<input type="button" value="不发布" onClick="notPass(this.form);"/>
...
[/@]
<script>
	var action = "${base}/cms/process-content!savePublish.action";
    function save(form){
    	if(!confirm("发布后网站上将会看到该信息，您是否确定要发布？")){
    		return false;
    	}
    	$(":button").each(function(){this.disabled=true});
    	form.action = action + "?publish.state=1";
        form.submit();
    }
    function back(form){
    	if($("#abstracts").val()==""){
    	...
    	$(":button").each(function(){this.disabled=true});
    	form.action = action + "?publish.state=0";
        form.submit();
    }
    function notPass(form){
    	...
    	$(":button").each(function(){this.disabled=true});
    	form.action = action + "?publish.state=2";
        form.submit();
    }
    function edit(form){
   		$(":button").each(function(){this.disabled=true});
    	form.action = "${base}/cms/process-content!publishAndEditContent.action";
