 ...
 <#macro thead extra...>
    <thead>
   <tr <#if (extra?size!=0)><#list extra?keys as attr>${attr}="${extra[attr]?html}"</#list></#if>><#nested></tr>
    </thead>
 </#macro>
 <#macro  tr class >
   <tr class="${class}" oclass="${class}" align="left" onmouseover="swapOverTR(this,this.className)"onmouseout="swapOutTR(this)" onclick="onRowChange(event)"><#nested></tr>
 </#macro>
 ...
 <#macro table extra...>
   ...
   <table class="${tableClass}" cellpadding="0" cellspacing="1" <#if (extra?size!=0)><#list extra?keys as attr>${attr}="${extra[attr]?html}" </#list></#if>>
   <#nested>
 ...
 <#macro tbody datas extra...>
    ...
    <#if thisPageSize??>
        <#if extra['simplePageBar']!false>
            <#include "/org/beangle/website/cms/template/template/simplePageBar.ftl"/>
        <#else>
            <#include "/org/beangle/website/cms/template/template/pageBar.ftl"/>
        </#if>
    </#if>
  </tbody>
</#macro>